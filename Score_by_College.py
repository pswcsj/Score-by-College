...
def byun_finder(uni, inquiry1, inquiry2):
    if (len(byun[byun['대학명']==uni]['계열코드'].unique()) == 2)| (len(byun[byun['대학명']==uni]['계열코드'].unique()) == 1):
...
def cal_score(korean, math, inquiry_std, inquiry_ba1, inquiry_ba2, english, history, score_by_college):
    ...
    #서강대
    su_byun = byun_finder('서강대', inquiry_ba1, inquiry_ba2)
    print(su_byun)
    print(korean*1.1 + math*1.3 + su_byun*0.6 + eng_su[english-1] + han_su[history-1])
    score_by_college['서강대'] = korean*1.1 + math*1.3 + su_byun*0.6 + eng_su[english-1] + han_su[history-1]
